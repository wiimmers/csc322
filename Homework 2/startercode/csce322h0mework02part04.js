    ...
    function whatever(moves)
	{
		...
		{ 
            if(x != 0 && y != 0 && x != (rows - 1) && y != (columns - 1))
            {
                if(i % numPlayers != 0)
                {
                    maze = newMaze(maze, (i % numPlayers), moves[i-1]);
                    location = playerLocation((i % numPlayers), maze);
                    x = location[0]; 
                    y = location[1];
                }
                ...
                {
                    maze = newMaze(maze, numPlayers, moves[i-1]);
                    location = playerLocation(numPlayers, maze);
                    x = location[0]; 
                    y = location[1];
                }
            }
